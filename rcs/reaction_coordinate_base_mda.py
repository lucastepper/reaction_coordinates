...
class ReactionCoordinate():
    ...
    def convert_nm(self, value: float|np.ndarray) -> float|np.ndarray:
        """ Convert a value from nm to Angstrom, as MDAnalysis uses Angstrom, but gmx nm. """
        return value / 10.

    def compute_com(self, idxs: Iterable[int]=None, selection: str=None, stride: int=1) -> np.ndarray:
        """ Compute the center of a group of atoms. Atoms can be selected either by
        indexes or selection string. If both are given, raise ValueError.
        Arguments:
            indexes: Indexes for the group of atom,
                can be list, range, int, np.ndarray, default is None
            selection: selection string for MDAnalysis, default: None
            stride: Stride for the trajectory, default: 1
        Returns:
            com(np.ndarray[3]): Center of mass for the group of atoms [nm]
        """
        if idxs is not None and selection is not None:
            raise ValueError('Only one of idxs and selection can be given.')
        if selection is not None:
            idxs = self.universe.select_atoms(selection).indices
        if idxs is None:
            idxs = np.arange(self.universe.atoms.n_atoms)
        com = np.zeros((len(self.universe.trajectory) // stride + 1, 3))
        for i, _ts in enumerate(self.universe.trajectory[::stride]):
            com[i] = self.universe.atoms[idxs].center_of_mass()
        # I saw that sometimes I need one more frame for com than len(traj) // stride
        # I am unsure if that laves the last frame of com zero some times, so I check here
        if np.all(com[-1] == 0):
            com = com[:-1]
        return self.convert_nm(com)
    ...
    def write_plumed(self, file_name, rc_file=None, append=False, overwrite=True, stride=1):
        ...
        if rc_file:
            ...
            lines.append(' \n')
        with open(file_name, 'a' if append else 'w', encoding="UTF-8") as fh:
            for line in lines:
                fh.write(line)
